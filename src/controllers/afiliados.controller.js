...
export const updateEdades = async (req, res) => {
  try {
    const afiliados = getConnection().data.afiliados;
    const updatedAfiliados = afiliados.map(a => {
      const edad = calculateAge(a.fecha_nac);
      return { ...a, edad};
    });
    
    getConnection().data.afiliados = updatedAfiliados;
    await getConnection().write();
    return res.json({ exito: `Edades actualizadas correctamente` });
  }
  catch (error) {
    return res.status(500).send({ message: error.message });
  }
